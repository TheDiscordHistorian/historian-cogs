...
from ..utility import format_description, format_date, format_media_type, format_anime_status, \
    format_manga_status, AniListSearchType, is_adult, clean_html
...
class Finder:
    ...
    async def get_aninews_embed(data: Dict[str, Any], page: int, pages: int) -> Embed:
        ...
        embed = discord.Embed(title=data.get('title'), url=data.get('link'), color=discord.Color.random(),
    ...
    async def get_crunchynews_embed(data: Dict[str, Any], page: int, pages: int) -> Embed:
        ...
        embed.set_author(
            name=f'Crunchyroll News | {data.get("date")}')
    ...
    async def anilist_search(self, ctx: Context, search: str, type_: str) -> Union[List[Embed], None]:
        ...
        try:
            if type_ == AniListSearchType.Anime:
                data = await self.anilist.media(search=search, page=1, perPage=15, type=type_.upper())
    ...
    async def anilist_random(self, ctx: Context, search: str, type_: str, format_in: List[str]) -> Union[Embed, None]:
        ...
        try:
            ...
            data = await self.anilist.genre(genre=search, page=1, perPage=1, type=type_,
                                                format_in=format_in)
            ...
            if data.get('data')['Page']['media'] is not None and len(data.get('data')['Page']['media']) > 0:
                ...
                data = await self.anilist.genre(genre=search, page=page, perPage=1, type=type_,
                                                    format_in=format_in)

            else:

                data = await self.anilist.tag(tag=search, page=1, perPage=1, type=type_,
                                                  format_in=format_in)

                if data.get('data')['Page']['media'] is not None and len(data.get('data')['Page']['media']) > 0:
                    page = random.randrange(1, data.get(
                        'data')['Page']['pageInfo']['lastPage'])
                    data = await self.anilist.tag(tag=search, page=page, perPage=1, type=type_,
                                                      format_in=format_in)
                else:
                    return None
    ...
    async def get_media_embed(data: Dict[str, Any], page: Optional[int] = None, pages: Optional[int] = None) -> Embed:
        ...
        embed = discord.Embed(description=format_description(data.get('description'), 400)
                              ...
                              colour=int('0x' + data.get('coverImage')
                                         ['color'].replace('#', ''), 0)
                              if data.get('coverImage')['color'] else discord.Color.random())
